...
func TestUG() {
	str := "ǫɢhel中文loɪɴʀʏʙʜʟʰʱʲʳʴʵʶʷʸˀˁ˙ˠˡˢˣˤ˪ΑΒ"
	gbk, err := Utf8ToGbk(str)
	CheckError(err)
	LogBlue(gbk, gbk == nil)
	var gbks []byte
	UGEncode(str, func(gbk []byte, s string) {
		LogGreen(s, gbk)
		gbks = append(gbks, gbk...)
	}, func(r rune, s string) {
		//LogRed(s, r)
		gbks = append(gbks, s...)
	})
	s, err := GbkToUtf8(gbks)
	LogGreen(len(gbks), s, err)
}
