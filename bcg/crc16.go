package bcg

import "encoding/binary"

var chCRCHTalbe = []byte{ // CRC 高位字节值表
	}
var chCRCLTalbe = []byte{ // CRC 低位字节值表
	}

// CRC16 函数返回源数据的 16 位校验码
func CRC16(data []byte) uint16 {
	chCRCHi := byte(0xFF)
	chCRCLo := byte(0xFF)

	count := len(data)
	for i := 0; i < count; i++ {
		wIndex := chCRCLo ^ data[i]
		chCRCLo = chCRCHi ^ chCRCHTalbe[wIndex]
		chCRCHi = chCRCLTalbe[wIndex]
	}

	return (uint16(chCRCHi) << 8) | uint16(chCRCLo)
}

// CRC16Bytes 函数返回字节形式的 16 位校验码
func CRC16Bytes(data []byte) []byte {
	crcV := CRC16(data)
	crc2 := make([]byte, 2)
	binary.LittleEndian.PutUint16(crc2, crcV)
	return crc2
}

// CRC16Check 校验CRC16，末尾2字节必须包含校验码
func CRC16Check(data []byte) bool {
	dl := len(data)
	if dl < 2 {
		return false
	}
	crc := CRC16Bytes(data[:dl-2])
	return crc[0] == data[dl-2] && crc[1] == data[dl-1]
}

// CRC16Append 直接把 2 字节的校验码添加到源数据末尾
func CRC16Append(data []byte) []byte {
	crc2 := CRC16Bytes(data)
	return append(data, crc2[0:2]...)
}
